...
import data_sources
import config
from models import AgronomicInfo
...
# --- Score Modifiers ---
TREND_MODIFIER = {"Rising": 1.2, "Slightly Rising": 1.1, "Stable": 1.0, "Falling": 0.8, "Volatile": 0.9}
COST_MODIFIER = {"Low": 1.2, "Low-Medium": 1.1, "Medium": 1.0, "Medium-High": 0.9, "High": 0.8}
# DEMAND_MODIFIER REMOVED
RISK_VOLATILITY_MODIFIER = {"High": 0.8, "Medium": 0.95, "Low": 1.05} # Less penalty for low vol, higher for high
...
def check_critical_factors(
    ...
    crop_status = {crop: {"status": "ok", "reason": None, "priority_subsidy": None} for crop in suitable_crops}
    ...
    for event in events:
        ...
        is_critical_event = severity == "high" or any(k in summary.lower() for k in ["ban", "severe outbreak", "major damage", "quarantine"])

        if applies_to_region and is_critical_event:
            log.warning(f"CRITICAL EVENT DETECTED in region {region}: {summary} (Severity: {severity})")
            # If event affects specific crops, eliminate them. If affects all (empty list), eliminate all in region.
            crops_to_eliminate = affected_crops if affected_crops else suitable_crops # If empty list means applies to all

            for crop in crops_to_eliminate:
                if crop in crop_status and crop_status[crop]["status"] == "ok":
                    log.warning(f"-> CRITICAL CUTOFF: Eliminating '{crop}' due to event: {summary}")
                    crop_status[crop]["status"] = "eliminated_event"
                    crop_status[crop]["reason"] = f"Critical Event ({severity}): {summary}"
                    eliminated_count += 1
    ...
    for crop in suitable_crops:
        ...
        relevant_subsidies = data_sources.get_subsidy_details_for_crop(crop, subsidies, region)

        agronomic_info: Optional[AgronomicInfo] = data_sources.get_crop_agronomic_data(crop)
        if not agronomic_info: continue # Should not happen if suitable_crops list is correct

        input_cost_cat = agronomic_info.input_cost_cat
        rough_cost = config.ROUGH_INPUT_COST_ESTIMATES.get(input_cost_cat, 20000) # Use estimate from config
        ...
        for sub in relevant_subsidies:
            ...
            if "% cost reduction" in benefit_summary or "% subsidy on input" in benefit_summary:
                try:
                    # Extract percentage (simple extraction, might need refinement)
                    percentage_str = benefit_summary.split('%')[0].split()[-1]
                    percentage = float(percentage_str)
                    if percentage >= config.HARD_CUTOFF_SUBSIDY_THRESHOLD * 100: # e.g., 80% or more
                        is_game_changer = True
                        log.debug(f"Game changer identified for '{crop}': High percentage subsidy ({percentage}%) found in '{sub['program']}'")
                except ValueError:
                    pass # Ignore if percentage cannot be parsed

            # Example 2: Estimated value is a large fraction of estimated input cost
            elif estimated_value and rough_cost > 0 and (estimated_value / rough_cost) >= config.HARD_CUTOFF_SUBSIDY_THRESHOLD:
                is_game_changer = True
                log.debug(f"Game changer identified for '{crop}': Estimated subsidy value (₹{estimated_value}) >= {config.HARD_CUTOFF_SUBSIDY_THRESHOLD*100}% of estimated cost (₹{rough_cost}) for '{sub['program']}'")


            # Example 3: Specific keywords indicating full coverage
            elif any(k in benefit_summary for k in ["free seed", "100% subsidy", "full cost coverage"]):
                is_game_changer = True
                log.debug(f"Game changer identified for '{crop}': Keyword indicates full coverage in '{sub['program']}'")


            if is_game_changer:
                # If a game-changer is found, mark the crop and store the subsidy info
                log.info(f"--> GAME CHANGER: Marking '{crop}' as high priority due to subsidy: {sub.get('program', 'N/A')}")
                crop_status[crop]["status"] = "high_priority_subsidy"
                crop_status[crop]["priority_subsidy"] = sub # Store the specific subsidy dict
...
def calculate_scores(
    ...
    news_headlines = [item.get("headline", "").lower() for item in retrieved_news_context]
    news_full_text = " ".join(news_headlines) # Combine headlines for keyword search

    # Positive keywords (increase score slightly)
    positive_keywords = ["export rise", "demand strong", "trade deal", "good monsoon", "msp hike", "price increase"]
    ...
    negative_keywords = ["ban", "concern", "pest outbreak", "drought", "poor monsoon", "import surge", "price fall", "farmer protest"]
    if any(k in news_full_text for k in negative_keywords):
        news_sentiment_modifier -= 0.15 # Higher penalty for negative news

    log.info(f"Calculated simple news sentiment modifier based on {len(news_headlines)} headlines: {news_sentiment_modifier:+.2f}")
    ...
    for crop in candidate_crops:
        ...
        if not data:
            log.warning(f"Missing detailed data for candidate crop '{crop}'. Skipping scoring.")
            continue

        agronomic_info: Optional[AgronomicInfo] = data.get("agronomic_info")
        if not agronomic_info:
            log.warning(f"Missing agronomic info for candidate crop '{crop}'. Skipping scoring.")
            continue

        # --- 1. Profitability Score ---
        price_forecast = data.get("price_forecast", {})
        cost_category = agronomic_info.input_cost_cat
        yield_factor = 1.0 # Base yield factor (could adjust based on yield estimate range later)
        ...
        if priority_subsidy:
            ...
            subsidy_details = f"High Priority Game-Changer: {priority_subsidy.get('program', 'N/A')}"
        ...
        inverted_volatility_mod = 1.0 + (1.0 - RISK_VOLATILITY_MODIFIER.get(price_volatility, 1.0)) # e.g. High Vol (0.8) -> 1 + (1-0.8) = 1.2
        risk_score = inverted_volatility_mod - news_sentiment_modifier # Subtract positive sentiment (reduces risk score), add negative (increases risk score)
        ...
        final_score_components = {
            ...
            "risk": (risk_score - 1.0) * WEIGHTS["risk"] # Apply risk weight to deviation from neutral (1.0)
        }
        ...
        log.info(f"    Profitability: {profit_score:.2f} (Weighted: {final_score_components['profitability']:.3f})")
        log.info(f"    Subsidy:       {subsidy_score:.2f} (Weighted: {final_score_components['subsidy']:.3f})")
        log.info(f"    Risk:          {risk_score:.2f} (Weighted Component: {final_score_components['risk']:.3f})")
    ...
    for crop, score, factors in scores:
        if score_range <= 0.001: # Avoid division by zero or near-zero if all scores are identical
