...
import data_sources
from models import ExplanationPoint, Recommendation, AgronomicInfo, PesticideSuggestion, NewsItemReference
from config import OLLAMA_MODEL, OLLAMA_BASE_URL, LLM_TEMPERATURE, LLM_TIMEOUT_SECONDS # Keep config imports
...
def analyze_news_context_with_llm(
    ...
    """
    log.info(f"Analyzing {len(top_news_headlines)} top news headlines with direct Ollama call for relevance to {len(candidate_crops)} crops...")
    ...
    headlines_numbered = "\n".join(f"{i}. {h}" for i, h in enumerate(top_news_headlines))
        ...
        if response and 'message' in response and 'content' in response['message']:
            ...
            try:
                ...
                if isinstance(llm_result, dict):
                    impact_summary = llm_result.get("overall_summary", default_impact_summary)
                        ...
                        if crop_name in news_mapping:
                            if isinstance(headline_indices, list):
                                ...
                                for idx in headline_indices:
                                    if isinstance(idx, int) and 0 <= idx < len(top_news_headlines):
                                        headlines_for_crop.append(top_news_headlines[idx])
                                    else:
                                        log.warning(f"Ollama returned invalid index '{idx}' for crop '{crop_name}'.")
                                news_mapping[crop_name] = headlines_for_crop[:3] # Limit to 3
                            else:
                                log.warning(f"Ollama returned non-list indices '{headline_indices}' for crop '{crop_name}'.")
                        else:
                             log.warning(f"Ollama returned relevance for an unexpected crop '{crop_name}'.")

                    log.info("Successfully parsed Ollama news context JSON analysis.")
                    return impact_summary, news_mapping
                else:
                    log.error(f"Ollama JSON output was not a dictionary: {type(llm_result)}")
         ...
         log.error(f"Ollama API Response Error (News Analysis): {e.status_code} - {e.error}")
         if e.status_code == 404: log.error(f"Model '{OLLAMA_MODEL}' not found at {OLLAMA_BASE_URL}. Pull it with `ollama pull {OLLAMA_MODEL}`.")
         return f"Ollama API Error: {e.error}", news_mapping
    except Exception as e:
        log.error(f"Error invoking direct Ollama call (News Analysis): {e}", exc_info=True)
...
def generate_explanation(crop_name: str, score: float, factors: Dict, context: Dict, status_info: Dict) -> List[ExplanationPoint]:
    ...
    profit_qualifier = 'strong' if profit_score_val > 1.1 else 'moderate' if profit_score_val > 0.9 else 'lower'
    ...
    points.append(ExplanationPoint(reason_type="Profitability Analysis", detail=profit_detail, ai_component="Predictive Analytics (Price), Heuristic Evaluation (Yield/Cost)"))
    ...
    points.append(ExplanationPoint(reason_type="Agronomic Suitability", detail=f"Good match for {soil_type} soil. Water needs ({water_needs}) considered against forecast.", ai_component="Knowledge-Based Rules (Soil), Weather Data Integration"))
    ...
    if subsidy_score_val > 1.0:
        ...
        points.append(ExplanationPoint(reason_type="Financial Incentive (Subsidy)", detail=subsidy_detail, ai_component=component))
    ...
    points.append(ExplanationPoint(reason_type="Risk Assessment", detail=risk_detail, ai_component="Risk Factor Analysis (Volatility, News Sentiment)"))

    points.append(ExplanationPoint(reason_type="Methodology Note", detail="Recommendation derived using a hybrid approach combining rules, predictive signals, scoring models, and a knowledge base.", ai_component="Hybrid AI System"))

    log.debug(f"[{crop_name}] Generated explanation points: {[p.reason_type for p in points]}")
    return points

def get_primary_risks(crop_name: str, factors: Dict, events: List[Dict], region_code: str) -> List[str]:
    ...
    if "high volatility" in risk_details_lower: risks.add("Potential high price volatility.")
    elif "medium volatility" in risk_details_lower: risks.add("Moderate price volatility expected.")
    elif "low volatility" in risk_details_lower: risks.add("Price volatility expected to be relatively low.")

    # 2. Risks from Agronomic Data
    if agronomic_info:
        if agronomic_info.common_pests: risks.add(f"Monitor common pests/diseases: {', '.join(agronomic_info.common_pests)}.")
        if agronomic_info.general_notes and "risk" in agronomic_info.general_notes.lower(): risks.add(f"Agronomic Note: {agronomic_info.general_notes}")
        if "Soybean" == crop_name: risks.add("Sensitivity to waterlogging, ensure good drainage.")
        if "Maize" in crop_name: risks.add("Yield sensitive to dry spells during tasseling/silking.")
        if "Cotton" == crop_name: risks.add("High input requirements & significant pest pressure (esp. Bollworm).")
        if "Paddy" in crop_name: risks.add("Requires significant water; dependent on monsoon/irrigation.")
        if "Sugarcane" == crop_name: risks.add("Long duration crop; high water/nutrient needs.")
        if "Chickpea" in crop_name: risks.add("Susceptible to wilt diseases in some soils.")
        if "Tomato" in crop_name: risks.add("Prone to various viral diseases (e.g., Leaf Curl).")
...
def generate_llm_summary(recommendations: List[Recommendation], news_impact_summary: str, weather_summary: str) -> Optional[str]:
    ...
    for rec in recommendations[:2]: # Limit context to top 2
        profit_text = next((p.detail.split(' is ')[-1].split(',')[0] for p in rec.explanation_points if p.reason_type == "Profitability Analysis"), "N/A")
        subsidy_text = next((p.detail for p in rec.explanation_points if "Subsidy" in p.reason_type), None)
        risk_text = rec.primary_risks[0].split(':')[0].replace('.', '') if rec.primary_risks else "standard risks"
    ...
    try:
        ...
        if response and 'message' in response and 'content' in response['message']:
            ...
            log.error("Ollama summary response structure was unexpected or missing content.")
