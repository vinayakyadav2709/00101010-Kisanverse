...
def initialize_vector_db_and_model():
    ...
    if collection and embedding_model:
        ...
        return True
    try:
        log.info(f"Initializing ChromaDB client with persistent path: {config.CHROMA_DB_PATH}")
        ...
        log.info(f"Collection '{config.CHROMA_COLLECTION_NAME}' loaded/created ({collection.count()} items existing).")
        ...
        log.critical(f"Failed to initialize ChromaDB or Embedding Model: {e}", exc_info=True)
        return False
...
def scrape_google_news(queries: List[str], lookback_days: int) -> List[Dict]:
    ...
    log.info(f"Scraping Google News for {len(queries)} queries (Lookback: {lookback_days} days)...")
    all_items = []
    processed_urls = set()
    ...
    for query in queries:
        ...
        try:
            response = requests.get(search_url, headers=HEADERS, timeout=config.SCRAPER_TIMEOUT_SECONDS)
            response.raise_for_status() # Raise HTTPError for bad responses (4xx or 5xx)
            soup = BeautifulSoup(response.text, 'lxml') # Use lxml for speed if installed

            articles = soup.find_all('article')
            log.info(f"Found {len(articles)} potential articles for query '{query}'")

            for article in articles:
                headline_tag = article.find('a', class_='JtKRv') or article.find('h3') or article.find('h4')
                link_tag = article.find('a', href=True)
                source_tag = article.find('div', class_='vr1PYe') or article.find('div', class_='wfBUxe')
                time_tag = article.find('time', datetime=True)
                # Try to get description snippet
                desc_tag = article.find('div', class_='Y3v8qd') or article.find('span', class_='rQMQod')
                ...
                if headline_tag and link_tag:
                    ...
                    published_date_iso: str = datetime.now(timezone.utc).isoformat() # Default to now if parsing fails
                    ...
                    if time_tag and time_tag.has_attr('datetime'):
                        try:
                            ...
                            log.warning(f"Could not parse datetime: {time_tag['datetime']} for headline: '{headline}'")
                            # Keep default published_date_iso (now)

                    # Filter by date (only if successfully parsed)
                    if published_date and published_date < cutoff_date:
                        log.debug(f"Skipping old item (Published: {published_date_iso}): '{headline}'")
                        continue
    ...
    log.info(f"Scraping finished. Found {len(all_items)} unique news items within lookback period.")
    return all_items
...
def store_news_in_chroma(news_items: List[Dict]):
    ...
    if not collection or not embedding_model:
        log.error("ChromaDB collection or embedding model not initialized. Cannot store news.")
        return
    if not news_items:
        log.warning("No news items provided to store.")
        return

    log.info(f"Attempting to store/update {len(news_items)} scraped items in ChromaDB collection '{config.CHROMA_COLLECTION_NAME}'...")
    added_count = 0
    updated_count = 0
    skipped_count = 0
    ids_to_upsert = []
    ...
    if urls_to_check:
        try:
            ...
            existing_items_response = collection.get(ids=urls_to_check, include=[]) # include=[] fetches only existence
            existing_ids = set(existing_items_response.get('ids', []))
            log.info(f"Checked {len(urls_to_check)} URLs against ChromaDB, {len(existing_ids)} already exist and will be updated/skipped if identical.")
        except Exception as e:
            log.error(f"Error checking existing items in ChromaDB: {e}. Proceeding with upsert, may overwrite.", exc_info=True)
    ...
    for item in news_items:
        ...
        if not url or not headline:
            log.warning(f"Skipping item due to missing URL or headline: {item.get('headline', 'N/A URL')}")
            skipped_count += 1
            continue
        ...
        try:
            ...
            if not embedding_model: raise ValueError("Embedding model not loaded.")
    ...
    if ids_to_upsert:
        log.info(f"Upserting {len(ids_to_upsert)} items into Chroma collection '{config.CHROMA_COLLECTION_NAME}' ({added_count} new, {updated_count} updates)...")
        try:
            collection.upsert(
                ids=ids_to_upsert,
            ...
            log.info(f"Batch upsert successful. Collection now has {collection.count()} items.")
        except Exception as e:
    ...
    log.info(f"Finished storing news. Added: {added_count}, Updated: {updated_count}, Skipped: {skipped_count}")
...
if __name__ == "__main__":
    ...
    if news_items:
        ...
        log.warning("No news items were scraped from Google News. ChromaDB not updated.")
