...
import models.llm.models as models
...
def load_databases():
    ...
    try:
        if os.path.exists(config.AGRONOMIC_DB_PATH):
            ...
            log.info(
                f"Loaded {len(AGRONOMIC_DATA)} crops from {config.AGRONOMIC_DB_PATH}"
            )
        else:
            log.error(
                f"Agronomic DB not found: {config.AGRONOMIC_DB_PATH}. Run fetch script."
            )
            AGRONOMIC_DATA = {}
    except Exception as e:
...
def get_yield_estimate(crop_name: str, soil_type: str) -> str:
    ...
    if agronomic_info and agronomic_info.typical_yield_range_q_acre:
        ...
        if soil_type not in agronomic_info.suitable_soils:
            low = max(
                1.0, low * 0.8
            )  # Reduce lower bound by 20%, ensure it's at least 1.0
            high *= 0.9  # Reduce upper bound by 10%
            log.debug(
                f"[{crop_name}] Soil '{soil_type}' not listed as suitable. Adjusted yield: {low:.1f}-{high:.1f} q/acre"
            )
...
def get_relevant_events(
    latitude: float, longitude: float, crops: List[str]
) -> List[Dict]:
    ...
    for crop in crops:
        ...
        if agronomic_info:
            ...
            if (
                "bollworm" in str(agronomic_info.common_pests).lower()
            ...
            if (
                "armyworm" in str(agronomic_info.common_pests).lower()
            ...
            if (
                "blight" in str(agronomic_info.common_pests).lower()
...
def recommend_pesticides(
    crop_name: str, risks: List[str]
) -> List[models.PesticideSuggestion]:
    """Recommends pesticides based on common pests and identified risks/alerts."""
    log.info(
        f"Recommending pesticides for '{crop_name}' based on common pests and risks: {risks}"
    )
    recommendations = {}  # Use dict to avoid duplicate chemicals, update targets/timing
    agronomic_info = AGRONOMIC_DATA.get(crop_name)
    common_pests = (
        [p.lower() for p in agronomic_info.common_pests] if agronomic_info else []
    )
    ...
    pest_map = {
        ...
        "Profenofos": {
            "girdle beetle": "Podding",
            "bollworm": "Boll Stage",
            "semilooper": "Vegetative",
        },
        "Imidacloprid": {
            "whitefly": "Seedling/Early",
            "aphids": "Seedling/Early",
            "shoot fly": "Seed Treatment",
            "termite": "Seed Treatment/Soil",
        },
        "Acetamiprid": {
            "whitefly": "Vegetative",
            "aphids": "Vegetative",
        ...
        "Emamectin Benzoate": {
            "armyworm": "Vegetative/Reproductive",
            "pod borer": "Flowering/Podding",
            "fruit borer": "Fruiting",
            "bollworm": "Boll Stage",
            "leaf folder": "Vegetative",
        },
        "Spinetoram": {
            "armyworm": "Vegetative",
            "thrips": "Flowering",
            "leaf miner": "Vegetative",
        },
        "Cartap Hydrochloride": {
            "stem borer": "Vegetative",
            "leaf folder": "Vegetative",
        },
        "Fipronil": {
            "stem borer": "Vegetative",
            "thrips": "Flowering",
            "termite": "Soil Application",
            "white grub": "Soil Application",
        },
        "Chlorantraniliprole (Coragen)": {
            "bollworm": "Flowering/Boll",
            "pod borer": "Flowering/Podding",
            "stem borer": "Early",
            "leaf folder": "Vegetative",
            "armyworm": "Vegetative",
        ...
        "Chlorpyrifos": {
            "termite": "Soil Application (Pre/Post-sowing)",
            "cutworm": "Soil Drench",
            "white grub": "Soil Drench",
            "stem borer": "Basal Spray",
        },
        # Fungicides (Add if needed, map to diseases)
        "Mancozeb": {"blight": "Preventative/Early", "rust": "Preventative/Early"},
        "Carbendazim": {
            "wilt": "Seed Treatment/Drench",
            "powdery mildew": "Foliar Spray",
        },
    ...
    def add_recommendation(chem, pest, stage, reason):
        ...
        if key not in recommendations:
            recommendations[key] = models.PesticideSuggestion(
                chemical_name=chem, target_pest=target_text, timing_stage=stage
            )
        else:
        ...
        for chem, targets in pest_map.items():
            for pest_keyword, stage in targets.items():
                if pest_keyword in risk_lower:
                    ...
                    add_recommendation(
                        chem, pest_keyword, stage, "alert"
                    )  # Mark as triggered by alert

    # Finalize and limit results
    final_list = sorted(list(recommendations.values()), key=lambda x: x.chemical_name)
    log.info(
        f"Suggested {len(final_list)} potential pesticide options for '{crop_name}'. Limiting to top 5."
    )
    # Log first few suggestions for debugging
    log.debug(
        f"Top suggestions for '{crop_name}': {[(p.chemical_name, p.target_pest, p.timing_stage) for p in final_list[:5]]}"
    )
    return final_list[:5]  # Return only top 5 suggestions


def get_subsidy_details_for_crop(
    crop_name: str, input_subsidies: List[Dict], region_code: str
) -> List[Dict]:
    """Filters input subsidies list for relevance to the specific crop and region."""
    log.info(
        f"Filtering {len(input_subsidies)} provided subsidies for '{crop_name}' in region '{region_code}'"
    )
    relevant = []
    crop_name_lower = crop_name.lower().split(" ")[
        0
    ]  # Use first word of crop name for matching
    ...
    for sub in input_subsidies:
        ...
        applies_to_region = (
            True  # Assume applies if locations field is missing or empty
        )
        ...
        if is_relevant_to_crop:
            log.debug(
                f"Subsidy '{sub.get('program', 'N/A')}' identified as relevant for '{crop_name}'."
            )
            ...
            relevant.append(
                {
                    ...
                    "estimated_value_inr": sub.get(
                        "value_estimate_inr"
                    ),  # Pass through estimate if available
                }
            )

    log.info(
        f"Found {len(relevant)} relevant subsidies for '{crop_name}' based on provided list."
    )
    return relevant
...
def generate_price_chart_data(crop_name: str, forecast: Dict) -> List[Dict]:
    ...
    base = int(
        random.uniform(1500, 4000) * base_mult
    )  # Wider range for base price simulation
    data = []

    # Simulate for future dates (e.g., ~4, 5, 6, 7 months from now)
    dates_to_plot = [
        (datetime.now().date() + timedelta(days=d)) for d in [120, 150, 180, 210]
    ]
    trend_factor = 1.0
    if "Rising" in forecast.get("trend", ""):
        trend_factor = 1.03  # Simulate 3% increase per period
    if "Falling" in forecast.get("trend", ""):
        trend_factor = 0.97  # Simulate 3% decrease per period
...
def generate_water_chart_data(crop_name: str) -> List[Dict]:
    ...
    base_need = base_need_map.get(
        agronomic_info.water_needs if agronomic_info else "Moderate", 2
    )
...
def generate_fertilizer_chart_data(crop_name: str) -> List[Dict]:
    ...
    schedule = [
        {
            "stage": "Basal",
            "timing": "At/Before Sowing",
            "nutrients": f"Foundation Dose ({fert_needs})",
        }
    ]

    # Add top dressing based on needs (example logic)
    if (
        "High N" in fert_needs
        or "Very High N" in fert_needs
        or "High NPK" in fert_needs
    ):
        schedule.append(
            {
                "stage": "Top Dress 1",
                "timing": "~25-40 DAS",
                "nutrients": "Nitrogen Focus (e.g., Urea)",
            }
        )  # Days After Sowing

    # Add second top dressing for demanding crops or specific needs
    if (
        "Very High NPK" in fert_needs
        or "High" in water_needs
        or "Micronutrients" in fert_needs
        or "Long duration" in (agronomic_info.general_notes or "")
    ):
        schedule.append(
            {
                "stage": "Top Dress 2 / Foliar",
                "timing": "~50-75 DAS",
                "nutrients": "NPK Balance / Potassium / Micronutrients as needed",
            }
        )
    ...
    if "Sulphur" in fert_needs:
        ...
        for item in schedule:
            if "Sulphur" in item["nutrients"]:
                found = True
                break
        if not found:
            schedule.append(
                {
                    "stage": "Nutrient Specific",
                    "timing": "Basal/Early",
                    "nutrients": "Sulphur Application",
                }
            )
