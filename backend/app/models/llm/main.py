...
import models.llm.models as models  # Contains InputParams
import models.llm.pipeline as pipeline  # Contains run_recommendation_pipeline
import models.llm.config as config  # Contains DB paths
import models.llm.data_sources as data_sources  # To explicitly load DBs before pipeline
    ...
    try:
        ...
        for sub_in in raw_data.get("subsidies", []):
            ...
            if adapted_sub["program"] and adapted_sub["benefits"]:
                ...
                log.warning(
                    f"Skipping subsidy due to missing program/benefits: {sub_in.get('$id', 'Unknown ID')}"
                )
        ...
        log.error(
            f"Type conversion error during input adaptation (e.g., land_size): {e}"
        )
        ...
        try:
            ...
            if recommendation_response.recommendations:
                ...
                for i, rec in enumerate(recommendation_response.recommendations):
                    print(
                        f"{i + 1}. {rec.crop_name} (Score: {rec.recommendation_score})"
                    )
                print("\n--- LLM Generated Summary ---")
                print(
                    recommendation_response.overall_llm_summary
                ...
                print(
                    f"Reason/Context: {recommendation_response.weather_context_summary or recommendation_response.overall_llm_summary}"
                )

        except Exception as e:
            log.error(
                f"Failed to save output JSON to {OUTPUT_FILE}: {e}", exc_info=True
            )
            print(
