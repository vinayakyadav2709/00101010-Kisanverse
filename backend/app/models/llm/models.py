...
class AgronomicInfo(BaseModel):
    ...
    water_needs: str = Field(
        default="Unknown",
        description="General water requirement category (e.g., Low, Moderate, High, Very High)",
    )
    input_cost_cat: str = Field(
        default="Medium", description="Category of input cost (e.g., Low, Medium, High)"
    )
    fertilizer_needs: str = Field(
        default="Standard NPK",
        description="General fertilizer requirements or specific notes",
    )
    common_pests: List[str] = Field(
        default_factory=list, description="List of common pests and diseases"
    )
    typical_yield_range_q_acre: Optional[Tuple[float, float]] = Field(
        default=None, description="Typical yield range in quintals per acre (low, high)"
    )
    general_notes: Optional[str] = Field(
        default=None, description="Other relevant notes (e.g., duration, sensitivities)"
    )
    ...
    def check_yield_range(cls, v: Optional[Tuple[float, float]]):
        if v is not None:
            if not isinstance(v, tuple) or len(v) != 2:
                raise ValueError(
                    "typical_yield_range_q_acre must be a tuple of two floats"
                )
            ...
            if v[0] > v[1]:
                raise ValueError(
                    "Yield range: low value cannot be greater than high value"
                )
        return v
...
class WeatherInfo(BaseModel):
    ...
    wind_max: Optional[float] = Field(
        default=None, description="Maximum wind speed (km/h)"
    )
    radiation: Optional[float] = Field(
        default=None, description="Surface shortwave radiation sum (MJ/m²)"
    )
...
class SubsidyInfo(BaseModel):
    ...
    type: str = Field(
        default="Unknown", description="Type of subsidy (e.g., financial, asset, input)"
    )
    ...
    locations: List[str] = Field(
        ...
        description="List of applicable location codes/names (if any)",
    )
    value_estimate_inr: Optional[float] = Field(
        default=None, description="Estimated monetary value in INR (if available)"
    )
...
class InputParams(BaseModel):
    ...
    planting_date_estimate: date = Field(
        ..., description="Estimated planting start date"
    )
    land_size_acres: float = Field(
        ..., gt=0, description="Size of the land plot in acres"
    )
    weather_forecast: List[WeatherInfo] = Field(
        default_factory=list, description="List of daily weather forecast data"
    )
    available_subsidies: List[SubsidyInfo] = Field(
        default_factory=list, description="List of potentially available subsidies"
    )
...
class ExplanationPoint(BaseModel):
    reason_type: str = Field(
        ..., description="Category of the explanation (e.g., Profitability, Agronomic)"
    )
    detail: str = Field(..., description="Specific explanation text")
    ai_component: str = Field(
        ...,
        description="Underlying AI technique involved (e.g., Rule-Based, Predictive)",
    )


class KeyMetrics(BaseModel):
    expected_yield_range: str = Field(
        ..., description="Estimated yield range string (e.g., '10.0-15.0 q/acre')"
    )
    price_forecast_trend: str = Field(
        ..., description="Simulated price trend (e.g., Rising, Stable)"
    )
    estimated_input_cost_category: str = Field(
        ..., description="Input cost category (e.g., Low, Medium)"
    )
    primary_fertilizer_needs: str = Field(
        ..., description="Summary of fertilizer needs"
    )
...
class PriceChartData(BaseModel):
    description: str = Field(
        ..., description="Title or description for the price chart"
    )
...
class WaterChartData(BaseModel):
    description: str = Field(
        ..., description="Title or description for the water chart"
    )
...
class FertilizerChartData(BaseModel):
    description: str = Field(
        ..., description="Title or description for the fertilizer chart"
    )
...
class PesticideSuggestion(BaseModel):
    ...
    target_pest: str = Field(
        ...,
        description="Pest/disease targeted, reason (e.g., 'bollworm (common)', 'blight (alert)')",
    )
...
class Recommendation(BaseModel):
    ...
    relevant_news: List[NewsItemReference] = Field(
        default_factory=list,
        description="List of relevant news items identified for this crop",
    )
...
class RecommendationResponse(BaseModel):
    ...
    weather_context_summary: str = Field(
        ..., description="Summary of the weather forecast's implications"
    )
    news_headlines_considered: List[str] = Field(
        default_factory=list,
        description="Headlines retrieved via RAG and considered by the LLM.",
    )
    news_impact_summary_llm: Optional[str] = Field(
        default=None, description="LLM-generated summary of the news context's impact."
    )
    overall_llm_summary: Optional[str] = Field(
        default=None, description="Final user-facing summary generated by LLM."
    )
