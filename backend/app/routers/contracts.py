...
def get_contracts(
    ...
    try:
        ...
        # Process contracts to convert dynamic_fields to JSON
        contracts = [
            {
                **doc,
                "dynamic_fields": json.loads(doc["dynamic_fields"])
                if "dynamic_fields" in doc and doc["dynamic_fields"]
                else None,
            }
...
def get_contract(contract_id: str, language: str = "english"):
    try:
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(
            status_code=500, detail=f"Error fetching contract: {str(e)}"
        )
